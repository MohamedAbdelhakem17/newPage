...
const UserOptions = () => {
    ...
    const handleChangeLanguage = () => {
        const lang = isEnglish ? Language.AR : Language.EN; // Toggle language
        changeLanguage(lang); // Call the provided function to update language
    };

    return (
        <div className={`d-flex justify-content-${isEnglish?"end":"start"} gap-2 align-items-center`}>
            ...
            <div onClick={handleChangeLanguage} className={`d-flex user-select-none  align-items-center gap-2 ${isDark ? "text-white" : "text-black"}`} style={{ cursor: "pointer", height: "25px" }}>
                ...
                <i className={`fa-solid fa-caret-down ${isDark ? "text-white" : "text-black"} h5`} ></i>
            </div>
        </div>
    )
}
